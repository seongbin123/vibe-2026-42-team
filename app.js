...
const NOTICES = [
  {
    id: 1,
    tag: '업데이트',
    title: `수지갑 ${APP_VERSION} 출시 안내`,
    date: '2026.05.16',
    body: `수지갑 베타 ${APP_VERSION}이 정식 출시되었습니다! 하루 가용금액 자동 계산, 수원대 학식 메뉴 연동, 소비 패턴 분석 기능을 지금 바로 사용해보세요. 앞으로도 지속적인 업데이트로 더 편리한 생활비 관리를 지원할 예정입니다.`,
  },
  {
    id: 2,
    tag: '학식',
...
// 생활비 변경 시점 이후 지출만 합산 (budgetSetAt 없으면 전체 포함)
function getBudgetSpent(d) {
  const since = d.budgetSetAt || 0;
  return onlyExpenses(d.expenses)
    .filter(e => !since || parseInt(e.id) >= since)
...
const EMOJIS = { 식비:'', 카페:'', 교통:'', 술자리:'', 구독:'', 쇼핑:'', 병원:'', 학업:'📚', 기타:'' };
const CAT_COLORS = { 식비:'#7C6CF4', 카페:'#FFB347', 교통:'#4ADE80', 술자리:'#FF5A5A', 구독:'#00D4FF', 쇼핑:'#FF69B4', 병원:'#FF6EB4', 학업:'#3182F6', 기타:'#9090A8' };
...
const CAT_ICONS_SVG = {
  ...
  '학업':   `<svg width="20" height="20" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M2 4h7c1 0 1.5.7 1.5 1.5V17c-.8-.3-1.7-.5-2.5-.5H2V4z"/><path d="M18 4h-7c-1 0-1.5.7-1.5 1.5V17c.8-.3 1.7-.5 2.5-.5H18V4z"/></svg>`,
...
function renderAnalysis() {
  ...
  renderSubscriptions();
  renderCategoryChart(onlyExpenses(d.expenses), d.subscriptions || []);
  ...
  rows += `<div class="pace-row"><span>예산 대비</span><span>+${fmt(projectedDaily)}</span></div>`;
...
function renderCategoryChart(expenses, subscriptions) {
  ...
  chart.innerHTML = Object.entries(totals)
    ...
    .map(([cat, amt]) => {
      ...
      const svgStyle = cat === '카페' ? 'margin-left:4px;margin-top:-1px' : ['쇼핑','기타','병원','구독','술자리'].includes(cat) ? 'margin-left:1px' : '';
    ...
    content.innerHTML = `
      <div class="menu-card${isToday ? ' today-menu' : ''}">
        ...
        <div class="menu-two-col">
          ...
          ${renderCol('교직원 식단 (한식 뷔페)', faculty)}
        </div>
      </div>`;
  } else {
...
function initSwipeClose(sheetEl, closeFn, overlayEl) {
  ...
  sheetEl.addEventListener('touchend', function(e) {
    ...
    if (shouldClose) {
      ...
      setTimeout(() => { closeFn(); sheetEl.style.transform = ''; sheetEl.style.transition = ''; if (overlayEl) { overlayEl.style.background = ''; overlayEl.style.transition = ''; } }, 280);
    } else {
